...
) -> ColorRGB
{
  ...
  for (var i: i32 = 0; i < lights_count; i++) {
    ...
    let shadow_origin = select(hit.position + hit.normal * 0.001, hit.position - hit.normal * 0.001, dot(light_direction, hit.normal) < 0); // checking if the point lies in the shadow of the light
